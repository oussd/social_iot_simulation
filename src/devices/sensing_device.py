...
from .device import Device, QoELevel # Assuming Device and QoELevel are in device.py
...
class SensingDevice(Device):
    ...
    def _perform_sense_action(self, expected_load_for_task: int) -> Dict[str, Any]:
        ...
        self.log_event(f"Sensed {self.sensor_type}: {value if isinstance(value, (int,float)) else str(value):.2f if isinstance(value,float) else str(value)} (Acc: {actual_accuracy:.2f}) in {action_duration_ms:.0f}ms, Load Consumed: {action_load_cost}", level='debug')
        return {
            'success': True, 
            'value': value, 
    ...
    def handle_request(self, from_device: Optional[Device], task_type: str, 
        ...
        if not base_acceptance_outcome.get('success'):
            # Base class already handled rejection and its QoS for self (e.g., if overloaded)
            # The returned 'measured_qos_for_requestor' from base_acceptance_outcome will be used by the simulation
            # for 'from_device' to update its trust in 'self'.
        ...
        if action_result.get('success'):
            self.log_event(f"EXECUTED '{task_type}' for {from_device.nameShort() if from_device else 'autonomous_task'}, val: {action_result.get('value', 'N/A')}")
            # Monetary reward
            ...
            if self.framework_variant in ["social_basic", "full_siot"]:
                 for rel in self.relationships.get('back_me', []):
                     ...
                     if backup_outcome.get('success'):
                         ...
                         # Simulation level metric update for successful backup

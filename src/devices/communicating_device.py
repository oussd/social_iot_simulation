...
class CommunicatingDevice(Device):
    def __init__(self, device_id: str, name: str, max_load: int = 100,
        ...
        super().__init__(device_id, name, max_load, framework_variant,
                         capabilities=["transmit", "connect", protocol], # Core capabilities
                         logger_instance=logger_instance,
                         current_minute_provider=current_minute_provider)
        self.protocol = protocol
        self.connected_devices: List[Device] = [] # List of devices this comm device has an active link to
        self.base_transmit_reward = 8 # Base reward for completing a transmission task

        # QoS parameters specific to communication
        if 'transmission_latency_ms' not in self.announced_qos:
             self.announced_qos['transmission_latency_ms'] = random.uniform(5, 25) # Network latency
             self.qoe_thresholds['transmission_latency_ms'] = {'sigma': 10.0, 'delta': 5.0}
        if 'message_delivery_rate' not in self.announced_qos: # Reliability of message delivery
            self.announced_qos['message_delivery_rate'] = random.uniform(0.97, 1.0)
            # Use task_success_rate thresholds if specific ones for delivery rate aren't defined
            self.qoe_thresholds['message_delivery_rate'] = self.qoe_thresholds.get('task_success_rate', {'sigma': 0.05, 'delta': 0.02})

    # add_worker and remove_worker are inherited from the base Device class.
    # CommunicatingDevice can act as a controller if needed, as it's not a SensingDevice.

    def connect_device(self, device_to_connect: Device) -> bool:
        """Establishes a simulated connection to another device."""
        if device_to_connect not in self.connected_devices:
            self.connected_devices.append(device_to_connect)
            self.log_event(f"Connected to {device_to_connect.nameShort()} via {self.protocol}", level='debug')
            return True
        return True # Already connected

    def _perform_transmit_action(self, message: str, target_device: Device, expected_load_for_task: int) -> Dict[str, Any]:
        """
        Simulates the transmission action and returns QoS metrics.
        expected_load_for_task: The load anticipated for this specific transmission.
        """
        ...
        if target_device not in self.connected_devices:
            self.log_event(f"Cannot transmit: {target_device.nameShort()} not in connected_devices list for {self.nameShort()}.", level='warning')
        ...
        if delivered_successfully:
            self.log_event(f"Transmitted '{message[:30]}...' to {target_device.nameShort()} via {self.protocol} in {action_duration_ms:.0f}ms, Load: {action_load_cost}", level='debug')
            # Optionally, log reception at the target device (if it has logging)
            if hasattr(target_device, 'log_event'):
                 target_device.log_event(f"Received '{message[:30]}...' from {self.nameShort()}", level='debug')
        else:
            self.log_event(f"Transmission of '{message[:30]}...' to {target_device.nameShort()} FAILED (simulated delivery failure).", level='warning')

        return {
            'success': delivered_successfully, 'load_consumed': action_load_cost,
            'processing_time_ms': action_duration_ms, # This is the device's internal action time for transmission
    ...
    def handle_request(self, from_device: Optional[Device], task_type: str,
        ...
        # Call base class handler for common checks
        ...
        # This device specializes in 'transmit' tasks
        ...
        target_comm_device = details.get('target_comm_device') # The ultimate recipient device

        if not target_comm_device or not isinstance(target_comm_device, Device):
            self.log_event(f"Invalid or no target_comm_device provided for transmit task.", level='warning')
            overall_response_time_ms = (time.time() - request_start_time) * 1000
        ...
        # Ensure connection to the target device (or its local communication interface if abstracted)
        self.connect_device(target_comm_device) # In a real system, this might involve routing
        ...
        measured_qos_for_trust_and_requestor = {
            ...
            'processing_time_ms': action_result.get('processing_time_ms', 0), # Latency of this comm device
        ...
        if action_result.get('success'):
            ...
            # Backup logic for communication
